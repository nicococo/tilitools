...
class ConvexSSAD:
    ...
    def fit(self):
        ...
        self.svs = []
        for i in range(N):
            if self.alphas[i] > ConvexSSAD.PRECISION:
                self.svs.append(i)
        ...
        for i in range(len(self.svs)):
            if thres[i, 0] < (T-ConvexSSAD.PRECISION):
                cnt += 1
    ...
    def calculate_threshold_dual(self):
        ...
        margins = []
        for i in self.svs:
            if self.alphas[i] < (self.cC[i,0] - ConvexSSAD.PRECISION):
                margins.append(i)
        ...
        k = self.kernel[margins, :]
        k = self.kernel[:, self.get_support_dual()]
        thres = self.apply(k)
