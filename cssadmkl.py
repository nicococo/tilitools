...
class CssadMKL:
    ...
    def __init__(self, X, y, kappa=1.0, Cp=1.0, Cu=1.0, Cn=1.0, ktype='mkl_rbf', param=[1.0]):
        ...
        self.cy = matrix(1.0,(1,self.samples),'d') # cy=+1.0 (unlabeled,pos) & cy=-1.0 (neg)
        self.cl = matrix(1.0,(1,self.samples),'d') # cl=+1.0 (labeled) cl=0.0 (unlabeled)
        self.cC = matrix(Cp,(self.samples,1),'d') # cC=Cu (unlabeled) cC=Cp (pos) cC=Cn (neg)
    ...
    def train_dual_mk(self):
        ...
        self.USE_MK=True
        self.mk_coef = [np.exp(-t*0.5) for t in range(len(self.kparam))]
        self.mk_coef /= sum(self.mk_coef)
        self.mk_coef = matrix(self.mk_coef)
        print(self.mk_coef)
    ...
    def train_dual_inner(self, P):
        ...
        #print(s)
        #P = Kernel.get_kernel(self.X, self.X, self.ktype, 0.01)

        # generate the final PDS kernel
        #Z = mul(Y,P) + 1.5*spmatrix(1.0, range(N), range(N))
        Z = mul(P,Y)

        #(w,v) = np.linalg.eig(np.array(Z)) 
        #w[w<0.0] = 0.0
        #print(w)
        #Z = matrix(np.diag(w)*v)
        
        #arr = np.array(Z)
        #print((arr.transpose() == arr).all())
        #print(arr)
        #print(np.linalg.eigvalsh(np.array(Z)))
        #np.savetxt('test.txt', np.array(Z), delimiter=',')
        #np.savetxt('test2.txt', np.array(self.X), delimiter=',')
        eigs = np.linalg.eigvalsh(np.array(Z))
        #print(np.linalg.eigvalsh(np.array(Z)))
        if (eigs[0]<0.0):
            print('Smallest eigenvalue is {0}'.format(eigs[0]))
            Z += spdiag([-eigs[0] for i in range(N)])
            print(np.linalg.eigvalsh(np.array(Z)))
        ...
        if (self.labeled==0):
            ...
            G  = sparse([G1,-G1])
            h  = matrix([h1,h2])
        #print(np.array(matrix(G), dtype=int))
    ...
    def apply_dual(self, Y):
        ...
        if self.USE_MK==True:
            ...
            print('Using local kernel refinement')
            t = self.mix
            P = matrix(1.0, (Ny,N))
            for i in range(len(self.kparam)):
                par = self.kparam[i]
